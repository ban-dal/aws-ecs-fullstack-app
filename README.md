...
앱 저장소의 `preprod` push는 해당 환경 이미지를 빌드해 ECS에 자동 배포한다. 앱 저장소의 `main`에서는 `Deploy app`을 수동 실행하고 `prod-deploy` 승인을 받으면 빌드부터 prod 배포까지 이어진다. 앱 역할은 각 환경의 ECR 저장소·ECS 서비스에만 접근한다. Terraform은 신규 생성용 태스크 정의를 관리하며 활성 ECS 서비스 revision은 앱 배포 workflow가 관리한다.
...
계정 ID와 state 버킷은 공개 Actions 로그에서 가리도록 secret으로 둔다. 환경 보호 규칙과 변수의 기대값은 [`scripts/check-github-settings.sh`](scripts/check-github-settings.sh)가 기준이다. 앱 저장소는 자체 `AWS_ACCOUNT_ID` secret, `AWS_REGION` variable, main 전용 `prod-deploy` 승인 환경을 사용한다.
