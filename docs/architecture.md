...
- **GitHub OIDC 역할** ([`iam-github-plan`](../infra/modules/iam-github-plan/main.tf), [`iam-github-apply`](../infra/modules/iam-github-apply/main.tf)): 저장소 immutable subject와 GitHub 환경 이름으로 신뢰를 제한하고, 환경별 plan·apply 역할이 자기 환경만 다룬다. apply 역할은 프로젝트가 쓰는 서비스를 서비스 단위로 허용하고, 다른 환경 리소스, 비용 상한을 넘는 호스트, 계정 전체 설정만 명시적으로 거부한다.
